...
Write-Host "Reportador ESCUCHADOR → cola real + despacho"
...
function Test-DispatchBusy {
  ...
  $proc = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'reportador\\tools\\dispatch-report\.mjs' }
  return [bool]$proc
}
...
while ($true) {
  ...
  Get-ChildItem $inbox -Filter '*-*.md' -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    Write-Host $it.text
    Show-Toast 'Reportador · pendiente' $it.text
  }
  ...
  foreach ($it in $items) {
    ...
    if (-not $it.working -and -not $anyWorking -and -not $busy -and -not $it.launched) {
      ...
      Show-Toast 'Reportador · trabajando' ($it.text.Substring(0, [Math]::Min(120, $it.text.Length)))
    ...
    if ($it.working -and $started -and ($now - $started).TotalMinutes -ge 15) {
      ...
      Show-Toast 'Reportador · trabado' "Lleva demasiado: $($it.id)"
