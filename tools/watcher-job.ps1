...
if ($Action -eq 'start') {
  ...
  Invoke-RestMethod -Uri 'http://127.0.0.1:8789/api/watcher/beat' -Method POST -ContentType 'application/json' -Body '{"status":"working"}' -TimeoutSec 4 | Out-Null
  ...
  Invoke-RestMethod -Uri 'http://127.0.0.1:8789/api/watcher/beat' -Method POST -ContentType 'application/json' -Body '{"status":"done"}' -TimeoutSec 4 | Out-Null
